...
// --- دالة إشعار تليجرام (منفصلة تماماً) ---
...
async function processPayment() {
    console.log("Starting payment process..."); // للفحص
    const btn = document.getElementById('payBtn');
    
    try {
        // جلب البيانات مع التأكد من وجود العناصر
        const amountElement = document.getElementById('amountDisplay');
        const phoneElement = document.getElementById('phone');
        const prodElement = document.getElementById('modalProdName');

        if (!amountElement || !phoneElement || !prodElement) {
            console.error("One or more HTML elements are missing!");
            return;
        }

        const amountVal = amountElement.value.replace(' SAR', '').trim();
        const phone = phoneElement.value;
        const prodName = prodElement.innerText;
        ...
        console.log("Bank Response:", data); // للفحص
