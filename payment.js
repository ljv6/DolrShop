...
async function processPayment() {
    // 1. جلب العناصر من الواجهة الجديدة
    ...
    if(!amountElement || !phoneInput || !nameElement) {
        alert("عذراً، حدث خطأ في تحميل بيانات المنتج");
        return;
    }

    // 2. تجهيز البيانات وتنظيف السعر
    const rawAmount = amountElement.innerText.replace(/[^0-9.]/g, '').trim();
    const formattedAmount = parseFloat(rawAmount).toFixed(2); // البنك يحتاج تنسيق 0.00
    ...
    // 4. إعداد بيانات التشفير والطلب
    const orderId = "DOLR-" + Date.now();
    const desc = "Order: " + prodName;

    // تشفير البيانات (MD5 ثم SHA1) كما تطلب Edfapay
    const md5Hash = md5((orderId + formattedAmount + "SAR" + desc + CONFIG.MERCHANT_PASSWORD).toUpperCase());
    ...
    try {
        ...
        if (data.redirect_url) {
            // التحويل لصفحة البنك (مدى / فيزا)
            window.location.href = data.redirect_url;
        } else {
            alert("خطأ من بوابة الدفع: " + (data.error_message || "يرجى التحقق من بيانات الحساب"));
            btn.disabled = false;
            btn.innerText = originalBtnText;
        }
    } catch (e) {
        alert("فشل الاتصال بخادم الدفع، يرجى المحاولة لاحقاً");
        btn.disabled = false;
        btn.innerText = originalBtnText;
    }
}

// --- دوال التشفير الضرورية (لا تحذفها) ---
